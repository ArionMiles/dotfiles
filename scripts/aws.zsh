# Source: https://github.com/ohmyzsh/ohmyzsh/blob/master/plugins/aws/aws.plugin.zsh
function agp() {
  echo $AWS_PROFILE
}
...
# AWS prompt configuration
# Colors: 208 = AWS Orange, 214 = lighter orange for profile, 222 = yellow-orange for region
AWS_PS1_SYMBOL="${AWS_PS1_SYMBOL:-\ue7ad}"
AWS_PS1_SYMBOL_COLOR="${AWS_PS1_SYMBOL_COLOR:-208}"
AWS_PS1_PROFILE_COLOR="${AWS_PS1_PROFILE_COLOR:-214}"
AWS_PS1_REGION_COLOR="${AWS_PS1_REGION_COLOR:-222}"
...
function aws_prompt_info() {
  ...
  # Return early if no profile is set
  ...
  # Prefix
  _aws_ps1+="${AWS_PS1_PREFIX}"

  # Symbol (AWS nerd font icon in orange)
  _aws_ps1+="%F{${AWS_PS1_SYMBOL_COLOR}}${AWS_PS1_SYMBOL}${reset_color}"

  # Separator
  _aws_ps1+="${AWS_PS1_SEPARATOR}"

  # Profile
  _aws_ps1+="%F{${AWS_PS1_PROFILE_COLOR}}${AWS_PROFILE}${reset_color}"

  # Region (if set)
  if [[ -n "$region" ]]; then
    _aws_ps1+="${AWS_PS1_DIVIDER}"
    _aws_ps1+="%F{${AWS_PS1_REGION_COLOR}}${region}${reset_color}"
  fi

  # Suffix
  _aws_ps1+="${AWS_PS1_SUFFIX}"
